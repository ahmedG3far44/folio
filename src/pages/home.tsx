...
function Home() {
  ...
  const { bio, experiences, projects, testimonials, skills, contacts } =
    useUser();
            ...
            ({
              ...
              return (
                <div
                  ...
                  aria-disabled={
                    activeTheme.id?.toString() === id?.toString() &&
                    activeTheme.themeName === themeName
                  }
                  className={`p-4 flex justify-center items-center  disabled:cursor-not-allowed rounded-md ${
                    activeTheme.id?.toString() === id?.toString() &&
                    activeTheme.themeName === themeName
                      ? "border-2 border-blue-500 rounded-md cursor-not-allowed bg-blue-500"
                      : "cursor-pointer hover:scale-95 duration-300 bg-zinc-700"
                  }`}
                >
                  <ThemeCard
                    key={id}
